...
try {
    ...
    */
    ...
    try {
        ...
        if((url != "")&&((url == "http://www.icbcasia.com/")||(url == "https://www.icbcasia.com/"))){
        if((location.href.indexOf("icbcrefer=")>0)||((GetAnalysisCookie('icbcrefer')!="")&&(GetAnalysisCookie('icbcrefer')!=null))){
          if(location.href.indexOf("icbcrefer=")>0){//在index.htm中重定向的页面
                url=location.href.substring(location.href.indexOf("icbcrefer=")+10);
            }else{
            url=unescape(GetAnalysisCookie('icbcrefer'));
            delCookie('icbcrefer');
        }
        }
        }
   ...
   if (path != "") {
        ...
        var urlHost=window.location.host;//获得当前域名
         //201812境外分行旧客行记录改https,modified by wangmin
        if(document.location.protocol=="https:")
            {
                path=changeHitHead(path);
            }
...
function ClientAnalysisADHit(obj) {
    ...
    if(path!=null && path.indexOf("https://")>-1)
    {
        nodeJSPath = "https://act.icbc.com.cn/image/hitcount.adv";
    }        
    //组织参数
    var nodeJSParas = obj;
    AjaxCallforNodeJS(nodeJSPath, nodeJSParas);
...
//删除cookies  
    function delCookie(name) {  
        ...
        var cval = GetAnalysisCookie(name);  
        if (cval != null)  
